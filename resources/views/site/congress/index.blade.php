                            ...
                            @foreach ($congress as $item)
                                    <div class="col-lg-6 col-md-12 col-12">
                                        ...
                                        <div class="single-event mb-50 mt-2">
                                            <div class="event-img">
                                                <a href="{!! url('/congresso/' . urlencode($item->title)) !!}">
                                                    <img class="img-fluid" src="/storage/{{ $item->image }}" alt="event">
                                                </a>
                                            </div>
                                            <div class="event-content text-start">

                                                <h4><a href="{!! url('/congresso/' . urlencode($item->title)) !!}">{!! html_entity_decode(mb_substr($item->title, 0, 30, 'UTF-8')) !!}</a></h4>
                                                <h3>{{ date('d-m-Y', strtotime($item->dateEvent)) }}</h3>
                                                <ul>
                                                    <li><i class="bi bi-clock "></i>{{ $item->startTime }} -{{ $item->endTime }}
                                                    </li>
                                                    <li><i class="bi bi-geo-alt-fill"></i>{{ $item->localization }}</li>
                                                </ul>
                                                <div class="event-content-right">
                                                    <a class="default-btn" href="{!! url('/congresso/' . urlencode($item->title)) !!}">Saber Mais</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
